...
Производится настройка беспарольного входа для УЗ, из под которых планируется выполнение операций с РК. Так как в задании указан системный каталог ```/etc```, ко многим вложенным ресурсам которого требуется доступ супeрпользователя, то 
доступ попутно настраивается и для УЗ **root**: в домашний каталог кладётся файл с зарание сгенерированным приватным ключом, устанавливаются соответствующие права на него и попутно прописывается ключ
